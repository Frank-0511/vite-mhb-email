...
describe("shouldRefreshCurrentTemplate", () => {
  ...
  test("refresca ante fuentes compartidas y configuraciones de Maizzle/Tailwind", () => {
    ...
    expect(shouldRefreshCurrentTemplate("maizzle.config.ts", "welcome")).toBe(true);
    expect(shouldRefreshCurrentTemplate("tailwind.email.config.ts", "welcome")).toBe(true);
  });
  ...
  test("no refresca ante fuentes irrelevantes fuera del scope de email", () => {
    expect(shouldRefreshCurrentTemplate("src/web/features/preview/main.ts", "welcome")).toBe(false);
...
describe("setupPreviewHmr", () => {
  type HmrEvent = { type: string; [key: string]: unknown };
  /**
   * Helper para armar dependencias mockeadas de HMR.
   */
  function createHmrTestHarness(overrides: Record<string, unknown> = {}) {
    const events: HmrEvent[] = [];
    const logs: unknown[] = [];

    /** @type {((payload?: { file?: string }) => Promise<void>) | null} */
    let registeredCallback: ((payload?: { file?: string }) => Promise<void>) | null = null;

    const hot = {
      on: (event: string, callback: (payload?: { file?: string }) => Promise<void>) => {
    ...
    const editorAPI = {
      setInitialData: (data: unknown) => events.push({ type: "editor.setInitialData", data }),
      updateContent: (data: unknown) => events.push({ type: "editor.updateContent", data }),
    };
    ...
    const renderAPI = {
      ...
      invalidateTemplateCache: (template: string) => {
        events.push({ type: "renderAPI.invalidateTemplateCache", template });
        return Promise.resolve();
      },
    };

    const fetchLatestData = (template: string) => {
    ...
    const deps = {
      ...
      ...overrides,
    } as Parameters<typeof setupPreviewHmr>[0];
  ...
  test("shared source refresca (layouts, partials, styles, config)", async () => {
    ...
    await harness.trigger("tailwind.email.config.ts");

    const invalidationEvents = harness.events.filter(
      (e: HmrEvent) => e.type === "renderAPI.invalidateTemplateCache",
    );
    const rerenderEvents = harness.events.filter(
      (e: HmrEvent) => e.type === "renderCurrentTemplate",
    );
  ...
  test("archivo de otro template no refresca", async () => {
    ...
    await harness.trigger("src/web/features/preview/main.ts");
