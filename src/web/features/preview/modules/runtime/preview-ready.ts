...
export function markPreviewReady(
  ...
  const skeletons = doc.querySelectorAll
    ? (doc.querySelectorAll("ef-skeleton") as NodeListOf<
        Element & { reveal?: (documentObj: Document) => void }
      >)
    : [];
  ...
  if (hasCustomSkeletons) {
    for (const skeleton of skeletons) {
      if (typeof skeleton.reveal === "function") {
        skeleton.reveal(doc);
      } else {
  ...
  const saveBtn = doc.getElementById("btn-save") as HTMLButtonElement | null;
  const resetBtn = doc.getElementById("btn-reset") as HTMLButtonElement | null;
