 ...
 * @property {{ createElement: (tagName: string) => any }} [document] - Objeto Document inyectable para testing.
 * @property {{ createObjectURL: (blob: any) => string, revokeObjectURL: (url: string) => void }} [urlApi] - API URL inyectable.
 * @property {new (parts: any[], options?: any) => any} [Blob] - Constructor Blob inyectable.
 */

/**
 * @typedef {{ ok: true, filename: string } | { ok: false, error: string }} DownloadHtmlResult
 */
...
export function downloadHtml(options) {
  const { templateName, html } = options ?? /** @type {any} */ ({});
